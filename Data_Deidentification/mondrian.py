...
class Partition(object):
    ...
    def __init__(self, data, low, high, vallist):
        """
...
def get_normalized_width(partition, index):
    ...
    return width * 1.0 / QI_RANGE[index] if width != 0.0 else 0.0
...
def choose_dimension_sequential(partition):
    """
    chooss next dimension sequentially
    """
    for dim in range(QI_LEN):
...
def find_median(partition, dim):
    ...
    if middle < GL_K or len(value_list) <= 1:
        try:
            return ('', '', value_list[0], value_list[-1],value_list)
        except IndexError:
            return ('', '', '', '', '')
    index = 0
    split_index = 0
    ...
    try:
        ...
        # e.g.[1, 2, 3, 4, 4, 4, 4]
        nextVal = splitVal
    return (splitVal, nextVal, value_list[0], value_list[-1], value_list)
...
def anonymize_strict(partition):
    ...
    for index in range(allow_count):
        ...
        dim = choose_dimension_sequential(partition)
        if dim == -1:
            print "Error: dim=-1"
            pdb.set_trace()
        (splitVal, nextVal, low, high,vallist) = find_median(partition, dim)
        # Update parent low and high
        if low is not '':
            partition.low[dim] = QI_DICT[dim][low]
            partition.high[dim] = QI_DICT[dim][high]
            partition.vallist[dim] = vallist
        ...
        lhs = Partition([], partition.low, lhs_high,lhs_high)
        rhs = Partition([], rhs_low, partition.high,partition.high)
        for record in partition.member:
...
def mondrian(data, k, relax=False, QI_num=-1):
    ...
    vallist = [(len(t) - 1) for t in QI_ORDER]
    whole_partition = Partition(data, low, high,vallist)
    # begin mondrian
    ...
    for partition in RESULT:
        ...
        for record in partition.member[:]:
            for index in range(QI_LEN):
                record[index] = partition.vallist[index]
